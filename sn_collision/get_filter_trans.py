...
def get_g_ztf_tc():
    filt_df = pd.read_csv(dirname + '/filter_data/ZTF/P48_g.dat', 
                          delim_whitespace=True, skiprows=[0], 
                          header=None, names = ['wavelength', 'transmission']
                          )
    return Filter("g_ztf", filt_df.wavelength.values, filt_df.transmission.values)

def get_r_ztf_tc():
    filt_df = pd.read_csv(dirname + '/filter_data/ZTF/P48_r.dat', 
                          delim_whitespace=True, skiprows=[0], 
                          header=None, names = ['wavelength', 'transmission']
                          )
    return Filter("r_ztf", filt_df.wavelength.values, filt_df.transmission.values)

def get_i_ztf_tc():
    filt_df = pd.read_csv(dirname + '/filter_data/ZTF/P48_i.dat', 
                          delim_whitespace=True, skiprows=[0], 
                          header=None, names = ['wavelength', 'transmission']
                          )
    return Filter("i_ztf", filt_df.wavelength.values, filt_df.transmission.values)
